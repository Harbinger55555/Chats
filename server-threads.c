...
void *send_msg(void *args) {
    ...
    while (1) {
        pthread_mutex_lock(&alive_mutex);           // LOCK
    ...
    pthread_mutex_unlock(&alive_mutex);             // LOCK
    ...
    while (1) {
        ...
        if (a->conn->alive == 1) {                  // UNLOCK
            ...
            if (recv(a->conn->sockfd, (void *) &tmp_buffer, a->size, 0) == 0) {
                ...
                pthread_mutex_unlock(&alive_mutex); // UNLOCK
            } else {
                acquire_exclusive();
                printf("Recieved: %s\n", tmp_buffer);
...
void init_client_conns() {
    pthread_mutex_init(&alive_mutex, NULL);         // Initialize the alive mutex
